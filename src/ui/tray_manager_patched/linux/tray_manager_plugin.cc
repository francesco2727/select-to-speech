...
static FlMethodResponse* set_icon(TrayManagerPlugin* self, FlValue* args) {
  ...
  // For absolute paths (e.g. "/path/to/icon.png"), split into directory and
  // basename-without-extension.  This mirrors how Chromium/Electron handles
  // tray icons on Linux: the directory is set as the icon theme path via
  // app_indicator_set_icon_theme_path() so that KDE's StatusNotifierItem
  // implementation can resolve the icon through its IconThemePath D-Bus
  // property.  Without this, the tray slot is registered but the icon image
  // is invisible on KDE Plasma (both Wayland and X11).
  gchar* icon_dir  = NULL;
  gchar* icon_name = NULL;
  if (icon_path[0] == '/') {
    icon_dir  = g_path_get_dirname(icon_path);
    icon_name = g_path_get_basename(icon_path);
    // Strip file extension (.png, .ico, …) — appindicator wants the bare name
    gchar* dot = g_strrstr(icon_name, ".");
    if (dot) *dot = '\0';
  }

  const char* effective_name = icon_name ? icon_name : icon_path;

  if (!indicator) {
    indicator = app_indicator_new(id, effective_name,
  ...
  app_indicator_set_icon_full(indicator, effective_name, "");
